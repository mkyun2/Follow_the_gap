...
//========================Obstacle,Clearance info =============================
    for(int i=0; i<num_readings; i++)
    {
        ...
        else if((Obstacle_flag == 0) && (Obstacle_flag_pre == 1))
        {
            ...
            //Obstacle_point_end[i] = i;// * 0.25 - 60;	// Obstacle Ending Point
            Distance_of_Obstacle[i] = ranges2[i-1];///////////////////////////////////////////////////////
            //i++;
        }
    ...
    float Min_Distance=6.0;
    float Now_Distance=0;
    float Gain_GapAngle = 5;
    ...
    ROS_INFO("size: %d",Clearance_StartAngle.size());
     ...
     {  
        ...
        ROS_INFO("Start Angle: %f, End Angle: %f",Clearance_StartAngle[i]-60+(yaw*180.0/M_PI),Clearance_EndAngle[i]-60+(yaw*180.0/M_PI));
    ...
    ROS_INFO("Gap Angle: %f, Min_Distance: %f",Max_GapAngle,Min_Distance);
    ROS_INFO("GoalAngle: %f Final_Angle: %f Heading: %f", GoalAngle,Final_Angle, yaw*180.0/M_PI);
